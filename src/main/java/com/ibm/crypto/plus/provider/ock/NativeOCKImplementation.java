    ...
    private static boolean loadIfExists(File libraryFile) {
        return SystemAccessUtils.doPrivileged(() -> {
            if (!libraryFile.exists()) {
                if (debug != null) {
                    debug.println("File not found: " + libraryFile.getAbsolutePath());
                }
                return false;
            }
            try {
                ...
                if (debug != null) {
                    debug.println("Loaded: " + libraryFile.getAbsolutePath());
                }
                return true;
            } catch (Throwable t) {
                if (debug != null) {
                    debug.println("Failed to load " + libraryFile.getAbsolutePath() + ": " + t);
                    t.printStackTrace();
                }
                return false;
            }
